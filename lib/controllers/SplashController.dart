...
class SplashController extends GetxController {
  ...
  Future<void> _initSplashFlow() async {
    ...
    // Check if user is already in guest mode before anything
    final prefs = await SharedPreferences.getInstance();
    if ((prefs.getBool('isGuest') ?? false) ||
        (prefs.getBool('skip') ?? false)) {
      if (kDebugMode) print('🚫 Splash aborted — Guest mode detected early');
      return;
    }
  ...
  Future<void> handleNavigation() async {
    ...
    // ✅ Hard guest mode check before doing anything
    if ((prefs.getBool('isGuest') ?? false) ||
        (prefs.getBool('skip') ?? false)) {
  ...
  void _goTo(Widget screen) {
    Future.microtask(() async {
      if (abortSplashFlow) return; // skip pressed mid-transition
