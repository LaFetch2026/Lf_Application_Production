...
public abstract class VideoPlayer implements VideoPlayerInstanceApi {
  ...
  // CRASH FIX: Guard flag to prevent double-dispose and race conditions with MediaCodec callbacks
  private boolean isDisposed = false;
  // CRASH FIX: Store listener reference to unregister before release
  ...
  public VideoPlayer(
    ...
    try {
      ...
      exoPlayer.prepare();
      // CRASH FIX: Store listener reference so we can unregister it before release
      exoPlayerEventListener = createExoPlayerEventListener(exoPlayer, surfaceProducer);
      exoPlayer.addListener(exoPlayerEventListener);
      setAudioAttributes(exoPlayer, options.mixWithOthers);
    } catch (Exception e) {
      // CRASH FIX: Catch any codec initialization failure so it doesn't become
      // a pending JNI exception on the callback thread
      if (exoPlayer != null) {
        exoPlayer.release();
        exoPlayer = null;
      }
      throw new IllegalStateException("ExoPlayer init failed: " + e.getMessage(), e);
    }
  }
  ...
  public void dispose() {
    // CRASH FIX: Guard against double-dispose which can cause MediaCodec crashes
    // when callbacks fire after the codec is released
    if (isDisposed) {
      return;
    }
    isDisposed = true;
    ...
    // CRASH FIX: Unregister listener before release to prevent callbacks
    // from accessing the codec after it's released
    if (exoPlayerEventListener != null && exoPlayer != null) {
      exoPlayer.removeListener(exoPlayerEventListener);
    }
    // CRASH FIX: Guard against null exoPlayer (can be null if init failed)
    if (exoPlayer != null) {
      exoPlayer.release();
    }
  }
